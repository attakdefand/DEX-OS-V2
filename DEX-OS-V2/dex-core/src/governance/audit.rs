...
impl AuditStore {
    ...
    pub fn ingest(
        ...
        if !blob.exists() {
            ...
            if existing.content_hash != hash {
                return Err(AuditError::ImmutableConflict);
            }
        }
    ...
    pub fn verify(&self, id: &str) -> Result<(), AuditError> {
        let rec = self
            .find_by_id(id)?
            .ok_or_else(|| AuditError::Io("evidence id not found".into()))?;
        let mut file = File::open(self.base_dir.join(&rec.content_hash))
            .map_err(|e| AuditError::Io(e.to_string()))?;
        let mut buf = Vec::new();
        ...
        if hash != rec.content_hash {
            return Err(AuditError::Io("content hash mismatch".into()));
        }
    ...
    fn find_by_id(&self, id: &str) -> Result<Option<EvidenceRecord>, AuditError> {
        let data =
            fs::read_to_string(&self.index_path).map_err(|e| AuditError::Io(e.to_string()))?;
