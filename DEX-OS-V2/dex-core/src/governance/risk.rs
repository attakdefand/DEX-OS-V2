...
impl<'a> RiskRegistry<'a> {
    pub fn new(index: &'a GovernanceReferenceIndex) -> Result<Self, RiskError> {
        let has = index.scenarios().iter().any(|s| {
            s.domain == GovernanceDomain::RiskExceptionManagement
        ...
        Ok(Self {
            index,
            state: RiskRegistryState::default(),
        })
    }
    ...
    pub fn add_risk(
        ...
        self.state.risks.push(RiskItem {
            id: id.into(),
            title: title.into(),
    ...
    pub fn mitigate_risk(&mut self, id: &str) {
        if let Some(r) = self.state.risks.iter_mut().find(|r| r.id == id) {
            ...
            self.state.notifications.push(Notification {
                message: format!("Risk {} mitigated", id),
            });
        }
    }
    ...
    pub fn approve_exception(
        ...
        if expected != evidence_artifact {
            return Err(RiskError::EvidenceMismatch {
                expected: expected.clone(),
                got: evidence_artifact.to_string(),
            });
        }
        if let Some(ex) = self
            .state
            ...
            .find(|e| e.id == exception_id)
        {
            ...
            self.state.notifications.push(Notification {
                message: format!("Exception {} approved by {}", exception_id, approver_login),
            });
        }
        Ok(())
    }
    ...
    pub fn save_exception_register(&self) -> Result<(), RiskError> {
        ...
        for ex in &self.state.exceptions {
            wtr.write_record(&[
                ...
                if ex.approved { "true" } else { "false" },
                ex.approver.as_deref().unwrap_or(""),
                ex.owner.as_str(),
                ex.timeframe.as_str(),
                ex.justification.as_str(),
                &ex.timestamp.to_string(),
            ])
            .map_err(|e| RiskError::Io(e.to_string()))?;
        }
    ...
    pub fn load_exception_register(&mut self) -> Result<(), RiskError> {
        ...
        for rec in rdr.deserialize::<ExceptionCsvRow>() {
            ...
            if self.state.exceptions.iter().any(|e| e.id == r.id) {
                continue;
            }
            self.state.exceptions.push(ExceptionRequest {
                id: r.id,
                ...
                approved: r.approved,
                approver: if r.approver.is_empty() {
                    None
                } else {
                    Some(r.approver)
                },
    ...
    pub fn open_risks_count(&self) -> usize {
        self.state.risks.iter().filter(|r| r.open).count()
    }
...
fn exception_register_path() -> String {
    env::var("EXCEPTION_REGISTER_PATH")
        .or_else(|_| env::var("CI_EXCEPTION_REGISTER_PATH"))
        .unwrap_or_else(|_| "exception_register.csv".to_string())
}
