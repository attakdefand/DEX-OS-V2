...
fn map_frameworks(s: &GovernanceScenario) -> Vec<FrameworkRef> {
    ...
    frameworks.push(FrameworkRef {
        ...
        controls: vec!["A.6.1".into(), "A.12.1".into()],
    });
    frameworks.push(FrameworkRef {
        name: "NIST SP 800-53".to_string(),
        controls: vec!["AC-2".into(), "CM-3".into()],
    });
    if s.enrichment.evidence.to_lowercase().contains("iso") {
        frameworks.push(FrameworkRef {
            name: "ISO Mapping".to_string(),
            controls: vec!["iso_nist_mapping.yaml".into()],
        });
    }
    frameworks
}
...
) -> Result<ComplianceReport, crate::governance::GovernanceReferenceError> {
    let scenarios = load_governance_reference()?;
    let mut entries = Vec::new();
    for s in scenarios
        .into_iter()
        .filter(|s| s.domain == GovernanceDomain::ComplianceRegulatoryAlignment)
    {
