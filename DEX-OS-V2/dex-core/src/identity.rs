...
use crate::security::quantum_signatures::{
    DilithiumLevel, DilithiumSignature, DilithiumSignatureEngine, DEFAULT_DILITHIUM_CONTEXT,
};
...
impl QuantumSecureCrypto {
    ...
    pub fn generate_dilithium_keypair() -> (Vec<u8>, Vec<u8>) {
        DilithiumSignatureEngine::generate_keypair(DilithiumLevel::default())
            .map(|kp| (kp.private_key, kp.public_key))
            // In the unlikely case generation fails, fall back to zeroized keys
            .unwrap_or((vec![0u8; 32], vec![0u8; 32]))
    }

    /// Sign data using quantum-secure signature scheme
    pub fn quantum_sign(private_key: &[u8], data: &[u8]) -> Vec<u8> {
        let keypair = match DilithiumSignatureEngine::keypair_from_private(
            private_key,
            DilithiumLevel::default(),
        ) {
            Ok(keypair) => keypair,
            Err(_) => return vec![],
        };

        DilithiumSignatureEngine::sign(data, &keypair, Some(DEFAULT_DILITHIUM_CONTEXT))
    ...
    pub fn quantum_verify(public_key: &[u8], data: &[u8], signature: &[u8]) -> bool {
        let signature_record = DilithiumSignature {
            signature: signature.to_vec(),
            public_key: public_key.to_vec(),
            message_hash: Sha3_256::digest(data).to_vec(),
            level: DilithiumLevel::default(),
            context: DEFAULT_DILITHIUM_CONTEXT.to_string(),
        };

        DilithiumSignatureEngine::verify(data, &signature_record).unwrap_or(false)
    }
