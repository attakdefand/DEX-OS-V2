use dex_core::governance::{
    AuditStore, GovernanceComponent, GovernanceDomain, GovernanceReferenceIndex,
};
...
fn audit_store_ingests_verifies_and_logs() {
    ...
    // Note: SecurityManager doesn't store events that can be retrieved,
    // it only logs them to stdout via log_event method
}