...
- Local compose (added in this repo):
...
- Services launched:
  ...
  - `dex-ui` on port 8080 (nginx serving the built UI)

Secrets are placeholders—set `JWT_SECRET` (and `TRADER_SECRETS` if needed) before real use.
