// Test the TestCoverageTracker with the actual security_tests_full.csv file
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Testing TestCoverageTracker with actual security_tests_full.csv file");
    println!("=====================================================================");

    // Try different possible paths for the CSV file
    let possible_paths = [
        "../.reference/security_tests_full.csv",
        "../../.reference/security_tests_full.csv",
        "../../../.reference/security_tests_full.csv",
        "d:/DEX-OS-V2/DEX-OS-V2/.reference/security_tests_full.csv",
    ];
    ...
    for path in &possible_paths {
        match File::open(path) {
            Ok(f) => {
                file = Some(f);
                println!("Successfully opened CSV file: {}", path);
                break;
            }
            Err(_) => {
    ...
    let file = match file {
        Some(f) => f,
        None => {
            println!("Could not find security_tests_full.csv file");
            return Ok(());
        }
    };
    ...
    for (index, line_result) in reader.lines().enumerate().skip(1) {
        ...
        // For demonstration, let's process a reasonable subset
        if index >= 1000 {
    ...
    // Mark some tests as executed (first 300)
    for i in 0..300.min(test_names.len()) {
    ...
    println!(
        "  Test 100 executed: {}",
        coverage_tracker.is_test_executed(&test_names[99])
    );
    println!(
        "  Test 500 executed: {}",
        coverage_tracker.is_test_executed(&test_names[499])
    );

    // Test a non-executed test (should likely return false, but might have false positive)
    if test_names.len() > 500 {
        println!(
            "  Test 600 executed: {}",
            coverage_tracker.is_test_executed(&test_names[599])
        );
    }

    println!(
        "\n✅ Successfully processed {} tests from security_tests_full.csv",
        test_names.len()
    );
