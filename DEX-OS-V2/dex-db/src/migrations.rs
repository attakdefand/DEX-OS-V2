...
pub fn get_migrations() -> Vec<Migration> {
    vec![
        ...
        Migration {
            version: 5,
            description: "Add foreign keys from trades to orders",
            sql: r#"
                DO $$
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.table_constraints
                        WHERE table_name = 'trades' AND constraint_name = 'fk_trades_maker_order'
                    ) THEN
                        ALTER TABLE trades
                        ADD CONSTRAINT fk_trades_maker_order
                        FOREIGN KEY (maker_order_id) REFERENCES orders(id);
                    END IF;
                    IF NOT EXISTS (
                        SELECT 1 FROM information_schema.table_constraints
                        WHERE table_name = 'trades' AND constraint_name = 'fk_trades_taker_order'
                    ) THEN
                        ALTER TABLE trades
                        ADD CONSTRAINT fk_trades_taker_order
