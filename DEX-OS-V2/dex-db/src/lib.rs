...
impl DatabaseManager {
    ...
    /// Get shard ID for a given key (stable hash-based sharding)
    fn get_shard_id(&self, key: u64) -> u64 {
        key % self.num_shards
    }

    /// Compute a stable 64-bit hash for a trader id for sharding
    fn hash_trader_id(trader_id: &str) -> u64 {
        // Fixed seed for deterministic hashing across runs
        let mut hasher = XxHash64::with_seed(0xD3A0_5D3Du64);
        hasher.write(trader_id.as_bytes());
        hasher.finish()
    }
    ...
    pub async fn save_order(&self, order: &Order) -> Result<(), DatabaseError> {
        // Determine which shard to use based on a stable hash of the trader ID
        let shard_hash = Self::hash_trader_id(&order.trader_id);
        let shard_id = self.get_shard_id(shard_hash);
    ...
    /// Create a new order, letting the database assign the ID, and return the new ID.
    pub async fn create_order(&self, order: &Order) -> Result<OrderId, DatabaseError> {
        // Determine shard based on trader id
        let shard_hash = Self::hash_trader_id(&order.trader_id);
        let shard_id = self.get_shard_id(shard_hash);
        let pool = self.get_pool_for_shard(shard_id);
        ...
        let row = query(
            r#"
            INSERT INTO orders (
                trader_id, base_token, quote_token, side, order_type, price, quantity, timestamp
    ...
    /// Create a new trade, letting the database assign the ID, and return the new ID.
    pub async fn create_trade(&self, trade: &Trade) -> Result<TradeId, DatabaseError> {
        let shard_id = self.get_shard_id(trade.maker_order_id);
        let pool = self.get_pool_for_shard(shard_id);
        ...
        let row = query(
            r#"
            INSERT INTO trades (
                maker_order_id, taker_order_id, base_token, quote_token, price, quantity, timestamp
