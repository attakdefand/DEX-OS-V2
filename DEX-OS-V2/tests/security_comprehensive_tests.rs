...
use dex_core::security::{EventType, SecurityManager, SeverityLevel};
...
fn test_security__comprehensive__integration__works__on_request() {
    ...
    let mut privacy_service = PrivacyProtectionService::new();
    ...
    let public_input = zk_proof.public_input.clone();
    ...
    sync_event_data.insert(
        "data_id".to_string(),
        "zk_proof_verification_001".to_string(),
    );
...
fn test_security__comprehensive__policy__enforces__on_request() {
    ...
    policy_data.insert(
        "policy".to_string(),
        "secret_knowledge_verification".to_string(),
    );
...
fn test_security__comprehensive__policy__validates__on_request() {
    ...
    validation_data.insert(
        "components_validated".to_string(),
        "zk_proof,gossip_sync".to_string(),
    );
...
fn test_security__comprehensive__policy__logs_evidence__on_request() {
    ...
    evidence_data.insert(
        ...
        "zk_proof_verification".to_string(),
    );
    evidence_data.insert("component".to_string(), "privacy_protection".to_string());
    evidence_data.insert("evidence".to_string(), evidence.clone());
    ...
    let evidence_event = audit_events
        .iter()
        .find(|e| e.description.contains("evidence logged"));
...
fn test_security__comprehensive__policy__detects__on_request() {
    ...
    let detection_event = alert_events
        .iter()
        .find(|e| e.description.contains("suspicious activity"));
