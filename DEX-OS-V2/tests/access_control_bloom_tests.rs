...
//!
//! This test file requires the dex-core crate to compile successfully.
//! The imported types ClassificationLevel and SecurityManager should be 
//! available from dex_core::security module.

use dex_core::security::{ClassificationLevel, SecurityManager};
...
fn test_bloom_filter_performance_optimization() {
    let mut manager = SecurityManager::new();
    let owner = "owner".to_string();
    let user1 = "user_1".to_string();  // Changed from "user1" to "user_1" to match the ACL pattern
