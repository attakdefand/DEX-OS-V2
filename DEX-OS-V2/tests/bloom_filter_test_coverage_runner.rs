...
pub fn run_security_tests_with_coverage_tracking(
) -> Result<TestCoverageStats, Box<dyn std::error::Error>> {
    ...
    println!(
        "Total security tests available: {}",
        coverage_stats.total_tests
    );
    println!(
        "Tests executed in this run: {}",
        coverage_stats.executed_tests
    );
    println!(
    ...
    println!(
        ...
        test_stats.average_duration_ms
    );
...
pub fn demonstrate_advanced_coverage_tracking() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Test '{}' executed {} times",
        test_name,
        coverage_tracker.get_execution_count(test_name)
    );
    ...
    println!(
        "Coverage statistics: {} executed out of {} total tests ({:.2}%)",
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ Processing of security_tests_full.csv data ({} tests)",
        coverage_stats.total_tests
    );
