...
#[test]
fn test_security__security__security__bplus_tree__certificate_management__enforces__on_request() {
...
#[test]
fn test_security__security__security__bplus_tree__certificate_management__validates__on_request() {
    let mut cert_manager = CertificateManager::new();
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
...
#[test]
fn test_security__security__security__bplus_tree__certificate_management__blocks__on_request() {
    let mut cert_manager = CertificateManager::new();
    ...
    assert!(matches!(
        result,
        Err(SecurityError::CertificateAlreadyExists(_))
    ));
    ...
    assert!(matches!(
        revoke_result,
        Err(SecurityError::CertificateNotFound(_))
    ));
    ...
    assert!(matches!(
        second_revoke_result,
        Err(SecurityError::CertificateAlreadyRevoked(_))
    ));
}

#[test]
fn test_security__security__security__bplus_tree__certificate_management__detects__on_request() {
    let mut cert_manager = CertificateManager::new();
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
...
#[test]
fn test_security__security__security__bplus_tree__certificate_management__logs_evidence__on_request(
) {
...
#[test]
fn test_security__security__security__bplus_tree__certificate_management__rotates__on_request() {
    let mut cert_manager = CertificateManager::new();
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
    ...
    let new_cert = Certificate {
        id: "rotating_cert_v2".to_string(), // New version of certificate
    ...
    // Verify the new certificate was stored and old one is still revoked
    if let Some(cert) = cert_manager.get_certificate("rotating_cert_v2") {
        assert_eq!(cert.issuer, "CA2");
        assert_eq!(cert.data, vec![9, 10, 11, 12]);
    } else {
        panic!("New certificate not found after rotation");
    }

    if let Some(old_cert) = cert_manager.get_certificate("rotating_cert") {
        assert!(old_cert.revoked);
    }
}
