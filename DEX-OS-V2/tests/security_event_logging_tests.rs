...
use dex_core::security::{EventType, SeverityLevel};
use std::collections::{HashMap, VecDeque};

/// Minimal event record used by the tests to validate event logging flows.
...
/// Simple in-memory logger to exercise the event logging scenarios.
struct EventLogger {
    capacity: usize,
...
impl EventLogger {
    ...
    fn log(
        ...
        let event_id = format!("event-{}", self.next_id);
        self.next_id += 1;
    ...
    fn get_events_by_type(&self, event_type: EventType) -> Vec<SecurityEvent> {
        self.events
            .iter()
    ...
    fn get_events_by_severity(&self, severity: SeverityLevel) -> Vec<SecurityEvent> {
        self.events
            .iter()
...
fn test_security__event_logging__policy__logs_evidence__on_request() {
    ...
    let events = logger.get_events_by_type(EventType::PolicyViolation);
