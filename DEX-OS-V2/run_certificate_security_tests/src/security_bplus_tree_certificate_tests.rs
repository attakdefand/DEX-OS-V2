...
pub fn test_security__security__security__bplus_tree__certificate_management__blocks__on_request() {
    ...
    match result {
        ...
        _ => panic!("Expected CertificateAlreadyExists error"),
    }
    ...
    assert!(second_revoke_result.is_err());
    match second_revoke_result {
        Err(SecurityError::CertificateAlreadyRevoked(_)) => {},
        _ => panic!("Expected CertificateAlreadyRevoked error"),
    }
}
...
pub fn test_security__security__security__bplus_tree__certificate_management__rotates__on_request()
{
    ...
    let new_cert = Certificate {
        id: "rotating_cert_new".to_string(), // Different ID for the new certificate
    ...
    // Verify the new certificate was added
    if let Some(cert) = cert_manager.get_certificate("rotating_cert_new") {
