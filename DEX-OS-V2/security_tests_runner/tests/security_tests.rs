...
use dex_core::security::{ClassificationLevel, EventType, SecurityManager, SeverityLevel};
...
mod tests {
    ...
    fn test_security__governance_and_policy__vault__enforces__on_request() {
        ...
        // We can't directly access the private key_rotation field, so we'll test
        // that the rotate_keys method worked by checking that we can rotate again
    ...
    fn test_security__governance_and_policy__key_manager__enforces__on_request() {
        ...
        // Rotate again to test key rotation functionality
        let result2 = security_manager.rotate_keys(user_id);
        assert!(result2.is_ok());
        assert_ne!(result.unwrap().public_key, result2.unwrap().public_key);
    }
    ...
    fn test_security__governance_and_policy__database__enforces__on_request() {
        ...
        // Test access control using the public API
        assert!(security_manager.check_data_access("public_data", &owner));
        assert!(!security_manager.check_data_access("secret_data", &user));
        assert!(security_manager.check_data_access("secret_data", &"authorized_user".to_string()));
    }
    ...
    fn test_security__secure_sdlc_and_supply_chain__policy__enforces__on_request() {
        ...
        // Verify certificate is valid using the public API
        assert!(security_manager.is_certificate_valid("component_cert"));
    }
    ...
    fn test_global_identity_security() {
        ...
        assert!(identity_manager
            .verify_biometric(&trader_id, "fingerprint", bio_data)
            .unwrap());
    }
    ...
    fn test_security__governance_and_policy__snapshot__enforces__on_request() {
        ...
        // Take a snapshot
        let result =
            snapshot_manager.take_snapshot("proposal_1".to_string(), voting_power, metadata);
    ...
    fn test_security__governance_and_policy__snapshot__validates__on_request() {
        ...
        // Get current timestamp for snapshot creation
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();

        // Create a proposal with voting times in the future
        let proposal = Proposal {
            ...
            voting_start: now + 10000, // Set to a future time after snapshot
            voting_end: now + 20000,   // Set to a future time after voting start
            votes: Votes {
            ...
            created_at: now - 1000, // Created before snapshot
        };
        ...
        // Take a snapshot (should be taken before voting starts)
        let snapshot_result =
            snapshot_manager.take_snapshot("proposal_1".to_string(), voting_power, metadata);
        ...
        assert!(is_valid, "Snapshot validation failed - check that snapshot was taken before voting started and proposal is active");
    }
    ...
    fn test_security__governance_and_policy__snapshot__rotates__on_request() {
        ...
        // Take another snapshot (simulating rotation) - we need to ensure different timestamps
        // Since the timestamp is in seconds, we need to wait at least 1 second
        ...
        let result2 =
            snapshot_manager.take_snapshot("proposal_1".to_string(), voting_power2, metadata2);
    ...
    fn test_security__governance_and_policy__snapshot__blocks__on_request() {
        use dex_core::snapshot::{SnapshotError, SnapshotManager, SnapshotMetadata};
        use dex_core::types::TraderId;
        use std::collections::HashMap;
        ...
        let result = snapshot_manager
            .calculate_voting_weight("nonexistent_snapshot", &"trader1".to_string());
    ...
    fn test_security__governance_and_policy__snapshot__detects__on_request() {
        ...
        let weight_result =
            snapshot_manager.calculate_voting_weight(&snapshot_id, &"trader2".to_string());
    ...
    fn test_security__governance_and_policy__snapshot__logs_evidence__on_request() {
        use dex_core::security::{EventType, SecurityManager};
        use dex_core::snapshot::{SnapshotManager, SnapshotMetadata};
        use std::collections::HashMap;
    ...
    fn test_security__governance_and_policy__keeper__blocks__on_request() {
        use dex_core::keeper::{HealthStatus, KeeperService};
        use std::collections::HashMap;
    ...
    fn test_security__governance_and_policy__indexer__enforces__on_request() {
        use dex_core::indexer::{DataFilter, FilterCriteria, IndexerService};
        use std::collections::HashMap;
    ...
    fn test_security__governance_and_policy__indexer__validates__on_request() {
        use dex_core::indexer::{DataFilter, FilterCriteria, IndexerError, IndexerService};
        use std::collections::HashMap;
    ...
    fn test_security__governance_and_policy__indexer__blocks__on_request() {
        use dex_core::indexer::{IndexerError, IndexerService};
        use std::collections::HashMap;
