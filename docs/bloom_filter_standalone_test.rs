//! Standalone test for Bloom Filter implementation
...
fn main() {
    // Run tests
    println!("Running Bloom Filter tests...");
    ...
    assert!(filter.might_contain("user1"));
    assert!(filter.might_contain("user2"));
    assert!(!filter.might_contain("user3"));
