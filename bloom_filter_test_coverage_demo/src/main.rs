...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Try to open the CSV file with multiple possible paths
    let possible_paths = [
    ...
    let _used_path = ""; // Suppress warning
    ...
    for (_index, line_result) in reader.lines().enumerate().skip(1) { // Suppress warning
