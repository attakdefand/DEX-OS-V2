...
class RustIntelligenceEngine {
    static async analyzeErrors(code: string, filePath: string): Promise<any> {
        try {
    ...
    private static parseRustErrors(output: string): any[] {
        const errors: any[] = [];
    ...
    private static categorizeRustError(code: string, message: string): string {
...
class SolidityIntelligenceEngine {
    ...
    private static parseSolidityErrors(output: string): any[] {
        const errors: any[] = [];
        const lines = output.split('\n');
    ...
    private static categorizeSolidityError(message: string): string {
        // Compiler errors
        if (message.includes("Compiler error") || message.includes("Compilation failed")) return "compiler_error";
        
        // Reentrancy vulnerabilities
        if (message.includes("reentrancy") || 
            message.includes("call.value") || message.includes("send") || message.includes("transfer")) return "reentrancy";
        
        // Parser errors
        if (message.includes("ParserError") || message.includes("Expected") || 
            message.includes("unexpected") || message.includes("missing")) return "parser";
        
        // Visibility issues
        if (message.includes("Visibility") || message.includes("must be declared")) return "visibility";
        
        // Undefined variables
        if (message.includes("undeclared identifier") || message.includes("not found")) return "undefined_variable";
        
        // Inheritance issues
        if (message.includes("override") || message.includes("inheritance")) return "inheritance";
        
        // Storage layout issues
        if (message.includes("storage")) return "storage_layout";
...
class MoveIntelligenceEngine {
    ...
    private static parseMoveErrors(output: string): any[] {
        const errors: any[] = [];
        const lines = output.split('\n');
    ...
    private static categorizeMoveError(message: string): string {
        ...
        if (message.includes("ability") || message.includes("copy") || message.includes("drop") || 
            ...
            (message.includes("missing") && message.includes("ability"))) return "ability_constraint";
        
        // Resource safety issues
        if (message.includes("resource") || message.includes("linear") || 
            message.includes("Linear") || message.includes("resource safety")) return "resource_safety";
        
        // Type mismatches
        if (message.includes("type") && message.includes("mismatch")) return "type_mismatch";
        
        // Abort code errors
        if (message.includes("abort") || message.includes("Abort")) return "abort_code";
...
class RustFixingEngine {
    static async fixErrors(code: string, errors: any[], filePath: string = ""): Promise<any> {
        let fixedCode = code;
...
class SolidityFixingEngine {
    static async fixErrors(code: string, errors: any[]): Promise<{ fixedCode: string; fixPlan: string }> {
        let fixedCode = code;
        let fixPlan = "Fix Plan:\n";
...
class RustDebuggingEngine {
    static async debug(code: string, filePath: string): Promise<any> {
...
class SecurePatchGenerator {
    static validatePatch(originalCode: string, fixedCode: string, language: string): boolean {
        // In a real implementation, this would run security checks
        return true;
    }
    
    static async runTests(language: string, code: string, filePath: string): Promise<any> {
        try {
            let result;
            switch (language) {
                case "rust":
...
class RepositoryIntegrationModule {
    static async scanProject(): Promise<any> {
        try {
    ...
    static async commitPatch(filePath: string, patchDescription: string): Promise<any> {
        try {
...
\`\`\``
            ...
            case "project.autofix": {
                const { code, language, filePath } = request.params.arguments as { code: string; language: string; filePath?: string };
...
\`\`\``
            ...
            case "patch.validate": {
                const { language, code, filePath } = request.params.arguments as { language: string; code: string; filePath?: string };
...
${testResult.errors || "None"}`
            ...
            case "patch.plan": {
                const { originalCode, fixedCode, language } = request.params.arguments as { originalCode: string; fixedCode: string; language: string };
