...
async function checkFile3Status() {
    console.log("🔍 Status Check for File 3: protection_tests_full_with_dsa_types.csv");
    console.log("==================================================================");
    
    try {
        // Path to the third file
        const filePath = path.join(__dirname, "..", "DEX-OS-V2", ".reference", "layers", "gold", "3. protection_tests_full_with_dsa_types.csv");
        ...
        if (fs.existsSync(filePath)) {
            ...
            if (lines.length > 6) {
                console.log(`   ... and ${lines.length - 6} more test cases`);
            }
        } else {
...
console.log("🔍 File 3 Status Checker");
console.log("======================");
checkFile3Status().catch(console.error);